...
class ResultsAdapter(private val context: Context,
    ...
    interface ClickListener {
        fun gotoMap(position: Int, coordinates: Coordinates)
    }
    ...
    override fun onBindViewHolder(holder: ResultsViewHolder, position: Int) {
       ...
       // holder.address.text = FirebaseUtils().getAddress(coordinates, geocoder = Geocoder(context, Locale.getDefault()))
       // holder.distance.text = FirebaseLocationUtils(activity).getDistance(coordinates, geocoder = Geocoder(context, Locale.getDefault()))
      //      .toString()
        //holder.total.text = totalPrice.toString() //TODO: create a FirebaseUtil() function for totalPrice by store
        holder.itemView.setOnClickListener{
            onCLick.gotoMap(position, coordinates)
        }
    }
